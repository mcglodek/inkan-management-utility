...
#[derive(Default)] pub struct CreateRedelegationScreen { menu_index: usize }
impl CreateRedelegationScreen { pub fn new() -> Self { Self::default() } }
...
impl ScreenWidget for CreateRedelegationScreen {
    ...
    fn draw(&self, f: &mut Frame<'_>, size: Rect, _ctx: &AppCtx) {
        ...
        let explanation_paras = [
            "Placeholder page for building a Re-Delegation transaction.",
            "Later: convenience to revoke then delegate within one flow if supported.",
        ];
