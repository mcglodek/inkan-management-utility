...
#[derive(Default)] pub struct CreatePermanentInvalidationScreen { menu_index: usize }
impl CreatePermanentInvalidationScreen { pub fn new() -> Self { Self::default() } }
...
impl ScreenWidget for CreatePermanentInvalidationScreen {
    ...
    fn draw(&self, f: &mut Frame<'_>, size: Rect, _ctx: &AppCtx) {
        ...
        let explanation_paras = [
            "Placeholder page for building a Permanent Invalidation transaction.",
            "Later: one-click builder for createPermanentInvalidationEvent.",
        ];
        ...
        let list_items = vec![ListItem::new(Line::from(vec![
            ...
            Span::raw(MenuItem::BackToAdvancedTools.label()),
        ]))];
