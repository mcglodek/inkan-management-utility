...
#[derive(Default)]
pub struct CreateDelegationScreen { menu_index: usize }
impl CreateDelegationScreen { pub fn new() -> Self { Self::default() } }
...
impl ScreenWidget for CreateDelegationScreen {
    ...
    fn draw(&self, f: &mut Frame<'_>, size: Rect, _ctx: &AppCtx) {
        ...
        let explanation_paras = [
            "Placeholder page for building a Delegation transaction.",
            "Later: construct calldata for createDelegationEvent (EIP-1559), export unsigned tx.",
        ];
    ...
    return Ok(t); // Esc -> Back
        ...
        match k.code {
            KeyCode::Up => { if self.menu_index == 0 { self.menu_index = 0; } } // only one item
            KeyCode::Down | KeyCode::Tab => { self.menu_index = 0; }
            KeyCode::Enter => { return Ok(Transition::Pop) }
            _ => {}
        }
        Ok(Transition::Stay)
    }
